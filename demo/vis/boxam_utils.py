...
class BoxAMDetectorWrapper(nn.Module):
    ...
    def __call__(self, *args, **kwargs):
        ...
        if self.is_need_loss:
            ...
            self.detector.neck.training = True
            ...
            loss = self.detector.forward(**data, mode='loss')

            return [loss]
        else:
            self.detector.neck.training = False
            with torch.no_grad():
                results = self.detector.test_step(self.input_data)
                #results = self.detector.forward(self.input_data)
                return results
...
class BoxAMDetectorVisualizer:
    ...
    def __call__(self, img, targets, aug_smooth=True, eigen_smooth=False):
    ...
    def show_am(self,
       ...
       #cv2.imwrite("x.png",renormalized_am)
